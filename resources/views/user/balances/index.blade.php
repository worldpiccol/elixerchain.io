...
@section('content')
    ...
    <div class="row">
        <div class="col-xl-12 col-md-12">
            <!-- card -->
            <div class="card card-h-100 bg-dark">
                <!-- card body -->
                <div class="card-body">
                    <div class="row align-items-center text-center">
                        <div class="col-12">
                            <span class="text-white mb-3 lh-1 d-block text-truncate"><strong>Asset Value</strong></span>
                            <h4 class="mb-3">
                                <span class="counter-value-usd text-white" >...</span>
                            </h4>
                        </div>
                    </div>
                </div><!-- end card body -->
    ...
    {{-- <div class="row mt-3 mb-3">
        <div class="col-md-12 align-items-center text-center">
            <div class="btn-group btn-group-example text-center mb-3" role="group">
                <a class="btn btn-block btn-outline-light w-xl" data-bs-toggle="modal" data-bs-target=".send">Send crypto</a>
                <a class="btn btn-block btn-outline-light w-xl" data-bs-toggle="modal" data-bs-target=".send">Send crypto</a>
                <a class="btn btn-block btn-outline-light w-xl" data-bs-toggle="modal" data-bs-target=".recieve">Recieve crypto</a>
            </div>
        </div>
    </div> --}}
    {{-- <div class="modal fade recieve Modal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Recieve - Select Crypto</h5>
                ...
                <div class="modal-body">
                    <div class="px-3" data-simplebar style="max-height: 352px;">
                        ...
                        <div class="list-group">
                            ...
                            <a href="" class="list-group-item list-group-item-action justify-content-evenly mb-2">
                                <div class="float-start">
                                    <img src="{{ $crypto->image_url  }}" width="30" alt="">
                                </div>
                                <span class="p-5">{{ $crypto->name }} - {{ $crypto->symbol }}</span>
                            </a>
                            @empty
    ...
    <div class="modal fade send Modal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Send - Select Crypto</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="px-3" data-simplebar style="max-height: 352px;">
        ...
        <div class="col-xl-8">
            <div style="border: none;" class="card">
                {{-- <div class="card-header align-items-center d-flex">
                    <h4 class="card-title mb-0 flex-grow-1">Assets</h4>
                </div><!-- end card header --> --}}

                <div class="card-body px-0">
                    <div class="px-1" data-simplebar style="max-height: 500px;">
                        <ul class="list-unstyled balance-list activity-wid mb-0">

                            @forelse ($wallets as $wallet)
                                <li class="activity-list activity-border" data-crypto="{{ $wallet->crypto->cmc_id }}" data-rate="{{ $wallet->crypto->cus_price }}">
                                    <div class="activity-icon avatar-md">
                                        <span style="background-color: white ! important;" class="avatar-title">
                                        <img src="{{ $wallet->crypto->image_url  }}" width="45" alt="">
                                        </span>
                                    </div>
                                    <div class="timeline-list-item">
                                        <div class="d-flex">
                                            <div class="flex-grow-1 overflow-hidden me-4">
                                                <h5 class="font-size-18 mb-1">{{ $wallet->crypto->name }}</h5>
                                            ...
                                            <div class="flex-shrink-0 text-end me-3">
                                                <h6 class="mb-1 asset-balance" balance="{{ $wallet->balance }}">{{ round($wallet->balance,6) }} <strong>{{ $wallet->crypto->symbol }}</strong></h6>
        ...
        <div class="col-xl-4">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('user.swap.store') }}">
                        @csrf
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select id="from-currency" name="from_crypto_balance_id" class="form-select from-select" style="max-width: 100px;">
                                ...
                                @foreach ($wallet_with_balance as $wallet_from)
                                    <option symbol-sw="{{ $wallet_from->crypto->symbol }}" balance-sw="{{ $wallet_from->balance }}" crypto-sw="{{ $wallet_from->crypto->cmc_id}}" value="{{ $wallet_from->id }}">{{ $wallet_from->crypto->symbol }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <div class="input-group mb-3">
                                ...
                                <select id="to-currency" name="to_crypto_id" class="form-select to-select" style="max-width: 100px;">
                                    ...
                                    @foreach ($cryptos as $crypto_to)
                                    <option {{ $crypto_to->symbol == 'USDT' ? 'selected' : ' '}} to-symbol-sw="{{ $crypto_to->symbol }}"  data-rate="{{ $crypto_to->cus_price }}" to-crypto-sw="{{ $crypto_to->cmc_id }}" value="{{ $crypto_to->id }}" >{{ $crypto_to->symbol }}</option>
                                    @endforeach
                                </select>
