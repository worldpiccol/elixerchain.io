...
@section('title', $scheme->crypto->symbol.'/USDT')
    ...
    <div class="col-md-12 ">
        <div class="col-md-4 float-end">
            <!-- TradingView Widget BEGIN -->
                ...
                "isTransparent": true,
                "height": 510,
                "symbol": "BINANCE:{{ $scheme->crypto->symbol }}USDT",
    ...
    <div class="col-md-8">
        <div class="card border-light">
          <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text"
                            class="form-control form-control-md" name="" value="{{ $scheme->min_amount }} USD" placeholder="" disabled>
                        <small id="helpId" class="form-text text-muted"></small>
                    </div>
                    <div class="form-group mt-1">
                        <label for="">Max Invest</label>
                        <input type="text"
                            class="form-control form-control-md" name="" value="{{ $scheme->max_amount }} USD" disabled>
                        <small id="helpId" class="form-text text-muted"></small>
                    </div>
                </div>
                <div class="col-md-6">
                   <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text"
                                class="form-control form-control-md" name="" value="{{ $scheme->rate }}%" disabled>
                            <small id="helpId" class="form-text text-muted"></small>
                        </div>
                   </div>
                   <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text"
                                class="form-control form-control-md" name="" value="{{ $scheme->terms }} {{ $scheme->term_type }}" disabled>
                            <small id="helpId" class="form-text text-muted"></small>
                        </div>
                   </div>
                   </div>
                    <div class="form-group mt-1">
                        ...
                        <input type="text"
                            class="form-control form-control-md invest-amount" name="" placeholder="{{ $scheme->min_amount }} USD">
           ...
           <div class="row">
                <div class="pt-2">
                    <span class="text-dark fw-bolder">Asset Balance: {{ $balance->balance  }} {{ $balance->crypto->symbol }} <span class="digits">~ {{ round($wallet_balance_usd_value , 2) }} USD</span></span>
...
<script>
    $(document).ready(function () {
        ...
        let usd_balance = "{{ $wallet_balance_usd_value }}";
        let min_invest = "{{ $scheme->min_amount }}";
        let max_invest = "{{ $scheme->max_amount }}";
        let crypto = "{{ $scheme->crypto->cmc_id }}";
        let scheme_time = "{{ $scheme->terms }}";
        let scheme_time_type = "{{ $scheme->term_typ }}";
    ...
    //    });
        ...
        let tradeDecider = (chart, user_decision) => {
            ...
            axios.get(`{{ url('ajax/store-trade') }}/{{ $scheme->id }}/${outcome}/${amount}/${open_price}/${close_price}`)
            .then(res => {
                if (res.data === 'logged') {
                    location.reload();
                }
       ...
       setResetListener();
        ...
        var tradeChart = new Chart (ctx, {
            ...
            data: {
                ...
                labels: [],
                datasets: [{
                    label: "{{ $scheme->crypto->symbol }}/USDT",
                    fill: true,
       ...
       let pricesGenerator = (current_price, time) => {
           ...
           setTimeout(() => {
            ...
            // console.log(data);
            let amount = $('.invest-amount').val();
            axios.get(`{{ url('ajax/debit-account') }}/{{ $scheme->id }}/${amount}`)
            .then(res => {
        ...
        let getRate = (crypto) =>{
            ...
            axios.get("{{ route('get.crypto', ['cryptos' => $scheme->crypto->cmc_id]) }}")
            .then(res => {
