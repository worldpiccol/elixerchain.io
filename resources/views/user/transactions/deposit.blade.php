        ...
        <div class="col-md-6 offset-md-3">
            <div class="card">
                <div class="card-body p-4">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <form id="swap-form" method="POST" action="{{ route('user.deposit.store') }}">
                                @csrf
                                <div id="deposit-crypto" class="mb-4">
                                    ...
                                    <select style="line-height: 40px !important;" class="form-control form-control-lg crypto-wallet" name="deposit_to_crypto" id="">
                                       @foreach ($cryptos as $crypto)
                                            <option {{ $crypto->symbo === 'BTC' ? 'selected':'' }} data-crypto="{{ $crypto->cmc_id }}" data-symbol="{{ $crypto->symbol }}" value="{{ $crypto->id }}">- {{ Str::upper($crypto->name) }} <span>WALLET</span></option>
                                       @endforeach
                                    </select>
                                </div>
                                <div class="mb-4">
