                ...
                <div class="card-body p-4">
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <form id="swap-form" method="POST" action="{{ route('user.withraw.store') }}">
                                @csrf
                                {{-- <div class="mb-4">
                                    <label for="example-text-input" class="form-label">WITHDRAW FROM:</label>

                                    <select style="line-height: 40px !important;" class="form-control form-control-lg deposit-to" name="withdraw_from_account" id="">
                                ...
                                <div id="deposit-crypto" class="mb-4">
                                   ...
                                   <select style="line-height: 40px !important;" id="from-currency" name="from_crypto_balance_id" class="form-select crypto-wallet" style="max-width: 100px;">
                                            ...
                                            @foreach ($crypto_balances as $wallet)
                                                <option symbol-sw="{{ $wallet->crypto->symbol }}" balance-sw="{{ $wallet->balance }}" crypto-sw="{{ $wallet->crypto->cmc_id}}" value="{{ $wallet->id }}">{{ $wallet->crypto->symbol }}</option>
                                            @endforeach
                                    </select>
                                </div>
                                {{-- <div id="usd-holder" class="mb-4">
                                    <label for="example-search-input" class="form-label">AMOUNT <small>USD</small> - <small class="text-info"><i>MIN 100 USD</i></small></label>
                                    <input id="usd-input" style="line-height: 40px !important;" class="form-control form-control-lg usd-amount" placeholder="{{ $main_bal }}" type="text" name="usd_amount">
